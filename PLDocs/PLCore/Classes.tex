...
In order to create an instance of a RTTI class, you first need to request the class description. This can be done using the method \emph{PLCore::ClassManager::GetClass} and a given class name including the namespace the class is in. If the requested class can't be found, for example because the class is within a currently not loaded plugin, a NULL pointer is returned. So, it's strongly recommended to always check for \emph{NULL} in here. After you've received the RTTI class description, you're able to use the method \emph{PLCore::Class::Create} to create an instance of this class. This instantiation may fail if there's no, or no proper constructor within the class, so, you may want to check for \emph{NULL}, too. Within the source~code~\ref{Code:RTTICreateClassInstance}, you can see the described process.
...
// Get RTTI class description
const PLCore::Class *pMyClass = PLCore::ClassManager::GetInstance()->GetClass("MyNamespace::MyClass");
if (pMyClass != NULL && pMyClass->IsDerivedFrom("MyBaseClass")) {
...
<Plugin Version="1" PixelLightVersion="PixelLight 0.9.4-R1">
  ...
  <Name>MyPlugin</Name>
  <Version>1.0</Version>
  <Vendor>Copyright (C) 2002-2010 by Me</Vendor>
...
if (pMyClass != NULL && pMyDerivedClass != NULL) {
...
if (pMyClass != NULL) {
	...
	if (pMyObject != NULL) {
		...
		if (pDynVar != NULL && pDynVar->GetType().IsEnumType()) {
