...
void StringTest::Test()
{
	...
	bResult = true;
	sString = "";
	if (sString[(uint32)0] != '\0') bResult = false;
	sString = "1234";
	if (sString[(uint32)0] != '1' || sString[(uint32)1] != '2' ||
		sString[(uint32)2] != '3' || sString[(uint32)3] != '4' || sString[(uint32)5] != '\0') bResult = false;
	EndTask(bResult);
