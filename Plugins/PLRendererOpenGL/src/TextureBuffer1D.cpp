			...
			if (pAPIPixelFormat) {
				...
				if (!m_nNumOfMipmaps && bMipmaps) {
					...
					if (bMipmaps && nSize != 1) {
						...
						// If we don't define all mipmap levels down to 1 'mysterious' graphics bugs may occur were it is not
...
bool TextureBuffer1D::Download(uint32 nMipmap, EPixelFormat nFormat, void *pData, uint8 nFace) const
{
	// Check parameters
	if (nMipmap > m_nNumOfMipmaps || nFormat == Unknown || !pData || nFace)
		return false; // Error!
