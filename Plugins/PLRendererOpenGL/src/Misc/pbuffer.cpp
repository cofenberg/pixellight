...
#endif
#ifdef LINUX
// [TODO] Always is used as log level in Log in an enum but via the glx header the Always is defined as an define
    ...
    m_bIsActive(false), m_bManaged(managed)
{
	...
	PL_LOG(Debug, String::Format("Declare a PBuffer with \"%s\" parameters", strMode))
    m_strMode = strMode;
...
bool PBuffer::Initialize(int iWidth, int iHeight, bool bShareContexts, bool bShareObjects)
{
    ...
    if (m_bSharedContext)
    {
        ...
        if (format == 0)
        {
            PL_LOG(Error, "PBuffer creation: GetPixelFormat() failed")
            return false;
        }
    }
    else
    ...
    if (!m_hPBuffer)
    {
        ...
        PL_LOG(Error, "PBuffer creation: wglCreatePbufferARB() failed")
        if (err == ERROR_INVALID_PIXEL_FORMAT)
        {
            PL_LOG(Error, "ERROR_INVALID_PIXEL_FORMAT")
        }
        else if (err == ERROR_NO_SYSTEM_RESOURCES)
        {
            PL_LOG(Error, "ERROR_NO_SYSTEM_RESOURCES")
        }
        else if (err == ERROR_INVALID_DATA)
        {
            PL_LOG(Error, "ERROR_INVALID_DATA")
        }
    ...
    if (!m_hDC)
    {
        PL_LOG(Error, "PBuffer creation: wglGetPbufferDCARB() failed")
        return false;
    }
    ...
    else
    {
        ...
        if (!m_hGLRC)
        {
            PL_LOG(Error, "PBuffer creation: wglCreateContext() failed")
            return false;
        }
        ...
        if(m_bShareObjects)
        {
            ...
            {
                PL_LOG(Error, "PBuffer: wglShareLists() failed")
                return false;
            }
        }
    }
    ...
    PL_LOG(Debug, String::Format("Created a %d x %d PBuffer", m_iWidth, m_iHeight))
    ...
    if (wglGetPixelFormatAttribivARB(m_hDC, format, 0, sizeof(iattributes) / sizeof(int), iattributes, ivalues)) {
      PL_LOG(Debug, String::Format("r:%d g:%d b:%d a:%d float:%d depth:%d samples:%d aux:%d",
...
/*            pfAttribList->push_back(WGL_RED_BITS_ARB);
		...
		else if (token == "rgb") PL_LOG(Warning, String::Format("Mistake in components definition (rgb + %d)", m_iNComponents))
		...
		else if (token == "rgba") PL_LOG(Warning, String::Format("Mistake in components definition (rgba + %d)", m_iNComponents))
		...
		else if (token == "alpha") PL_LOG(Warning, String::Format("Mistake in components definition (alpha + %d)", m_iNComponents))
		...
		else if (token == "r") PL_LOG(Warning, String::Format("Mistake in components definition (r + %d)", m_iNComponents))
		...
		else if (token == "r") PL_LOG(Warning, String::Format("Mistake in components definition (rg + %d)", m_iNComponents))
...
//                pbAttribList->push_back(WGL_TEXTURE_RECTANGLE);
            ...
            if (bIsFloatBuffer || bIsATIFloatBuffer)
            {
				if(m_iNComponents == 0)
				{
					PL_LOG(Debug, "Components not specified. assuming rgba...")
					pfAttribList->push_back(WGL_RED_BITS_ARB);
					pfAttribList->push_back(m_iBitsPerComponent);
					pfAttribList->push_back(WGL_GREEN_BITS_ARB);
					pfAttribList->push_back(m_iBitsPerComponent);
            ...
            if (bIsFloatBuffer) {
				...
				case 4:
					...
					break;
				default:
			        PL_LOG(Error, String::Format("Bad number of components (r=1,rg=2,rgb=3,rgba=4): %d", m_iNComponents))
					break;
				}
            } 
            else
        ...
        PL_LOG(Error, String::Format("Unknown PBuffer attribute: %s", token.c_str()))
    }
...
int PBuffer::Bind(int iBuffer, int nFace)
{
    if (!m_bIsTexture)
    {
//        PL_LOG(Error, "PBuffer::Bind() failed - PBuffer format does not support render to texture!")
        return 0;
    }
    ...
    if (m_bIsBound)
    {
//        PL_LOG(Error, "PBuffer::Bind() failed - PBuffer is already bound")
        return 0;
    }
#endif

	// Set target cube map face
	if (nFace >= 0) {
		vector<int> lstAttribList;
		lstAttribList.push_back(WGL_CUBE_MAP_FACE_ARB);
		lstAttribList.push_back(WGL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB+nFace);
		lstAttribList.push_back(0);
		lstAttribList.push_back(0);
		wglSetPbufferAttribARB(m_hPBuffer, &lstAttribList[0]);
	}

    int ret = wglBindTexImageARB(m_hPBuffer, iBuffer);
//    if (!ret)
//        PL_LOG(Error, "PBuffer::Bind() failed")
...
int PBuffer::Release(int iBuffer, int nFace)
{
    if (!m_bIsTexture)
    {
//        PL_LOG(Error, "PBuffer::Release() failed - PBuffer format does not support render to texture!")
        return 0;
    }
    ...
    if (!m_bIsBound)
    {
//        PL_LOG(Error, "PBuffer::Release() failed - PBuffer is not bound")
        return 0;
    }
#endif
...
//	if (!ret)
//        PL_LOG(Error, "PBuffer::Release() failed")
...
//    if (!wglMakeCurrent(m_hDC, m_hGLRC))
  //      PL_LOG(Error, "PBuffer::Activate() failed")
...
//        PL_LOG(Error, "PBuffer::Deactivate() failed")
...
PBuffer::PBuffer(const char *strMode, bool managed) 
  : m_pDisplay(0), m_glxPbuffer(0), m_glxContext(0), m_pOldDisplay(0), m_glxOldDrawable(0), 
    ...
    m_bSharedContext(false), m_bShareObjects(false), m_bIsBound(false), m_bManaged(managed)
{
    m_pfAttribList.push_back(GLX_DRAWABLE_TYPE);
    m_pfAttribList.push_back(GLX_PBUFFER_BIT | GLX_WINDOW_BIT);
...
bool PBuffer::Initialize(int iWidth, int iHeight, bool bShareContexts, bool bShareObjects)
{
    ...
    if (m_bSharedContext)
    {
        ...
        if (!glxConfig)
        {
            PL_LOG(Error, "PBuffer creation: glXGetFBConfigs() failed")
            return false;
        }
    }  
    else
    {
        ...
        if (!glxConfig)
        {
            PL_LOG(Error, "PBuffer creation: glXChooseFBConfig() failed")
            return false;
        }
    }
    ...
    if (!m_glxPbuffer)
    {
        PL_LOG(Error, "PBuffer creation: glXCreatePbuffer() failed")
        return false;
    }
    ...
    else
    {
        ...
        if (!glxConfig)
        {
            PL_LOG(Error, "PBuffer creation: glXCreateNewContext() failed")
            return false;
        }
    }
...
void PBuffer::parseModeString(const char *modeString, vector<int> *pfAttribList, vector<int> *pbAttribList)
{
    ...
    for (unsigned int i = 0; i < tokens.size(); i++)
    {
		...
		else if (token == "rgb") PL_LOG(Debug, String::Format("Mistake in components definition (rgb + %d)", m_iNComponents))
...
#endif
	default:
		PL_LOG(Error, "Unknown m_iBitsPerComponent")
/* [TODO] This is not working on 64 bit, do we really need it?
#	if defined(WIN32)
		_asm { int 3 }
#	endif
		*/
