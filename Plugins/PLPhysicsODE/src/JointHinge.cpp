	...
	PLPhysics::JointHinge(cWorld, static_cast<World&>(cWorld).CreateJointImpl(), pParentBody, pChildBody, vPivotPoint, vPinDir)
{
	...
	dJointID pODEJoint = dJointCreateHinge(pODEWorld, 0);
	dJointSetHingeAnchor(pODEJoint, m_vPivotPoint.x, m_vPivotPoint.y, m_vPivotPoint.z);
	dJointSetHingeAxis(pODEJoint, m_vPinDir.x, m_vPinDir.y, m_vPinDir.z);
