	...
	EventHandlerKeyDown(&Application::NotifyKeyDown, this)
{
	...
	SetTitle(PLT("PL basic sound demo"));
	...
	if (m_sSoundAPI.GetLength()) {
		...
		if (pSceneContainerNode && pSceneContainerNode->IsInstanceOf("PLScene::SceneContainer")) {
			...
			if (pSoldier) {
				...
				pSoldier->AddModifier("PLScene::SNMPositionPath", "Filename=\"Data/Misc/PLDemoSoundBasic_Soldier.path\" Speed=\"0.5\" Flags=\"NodeIndexProgress\" Interpolation=\"CatmullRomCurve\"");
