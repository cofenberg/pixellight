...
This section deals with the different sound backends shipping with the PixelLight \ac{SDK}.
...
\item Used \ac{DLL}s: \emph{PLSoundNull.dll}
\end{itemize}
...
\item Used \ac{DLL}s: \emph{PLSoundOpenAL.dll}, \emph{OpenAL32.dll} and \emph{wrap\_oal.dll}
\end{itemize}


\paragraph{OpenSL ES}
\begin{itemize}
\item OpenSL ES\footnote{\url{http://www.khronos.org/registry/sles/}} backend for mobile devices.
\item PixelLight component name: PLSoundOpenSLES
\end{itemize}
...
\item Used \ac{DLL}s: \emph{PLSoundFMODEx.dll} and \emph{fmodex.dll}
\end{itemize}
...
\item Used \ac{DLL}s: \emph{PLSoundFMOD.dll} and \emph{fmod.dll}
\end{itemize}
