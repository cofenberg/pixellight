...
The latter option has the advantage that you can also build individual projects rather than only the whole \ac{SDK}. To build projects individually, change into the \emph{build-Debug} or \emph{build-Release} directory and type
\begin{lstlisting}[language=sh]
make <project name>
\end{lstlisting}
...
\subsection{2. Install the PixelLight \ac{SDK} Locally}
