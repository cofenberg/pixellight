...
\item{\emph{texlive} (\LaTeX{} system for compiling the documentation) - Ensure that \emph{dvipdfmx} is installed which is included within the \emph{texlive-base"-package} (e.g. \emph{dvipdf} will probably fail) Note: under gentoo dvipdfmx is a separate package}
...
\item{\emph{dvipdfm} (\LaTeX{} package for compiling the documentation, maybe only gentoo related separate package)}
