...
\end{itemize}
If you stick to those, you should be on the safe side. Other compilers and compiler versions may work as well, but are untested. \SI{64}{\bit} may work e.g. for \ac{MS} Windows and Linux, there are some pre-packed external \SI{64}{\bit} packages available as well. But please note that, at the moment, \SI{64}{\bit} is not officially supported. So if you encounter issues when using \SI{64}{\bit} please provide us with feedback so we can fix it.
