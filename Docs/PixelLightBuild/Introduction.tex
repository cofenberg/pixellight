...
In general, if you encounter time consuming pitfalls not yet described in this document, please tell us by using e.g. the official forum at \url{http://pixellight.sourceforge.net/forum/} or the bugtracker at \url{http://sourceforge.net/p/pixellight/bugs/}. If there's no feedback, we can't improve things to make it even easier to use PixelLight in the future.
...
git clone git://git.code.sf.net/p/pixellight/code
\end{lstlisting}

In case you're a PixelLight team member, you might want to clone by using your SourceForge username by writing
\begin{lstlisting}[language=sh]
git clone ssh://<user name>@git.code.sf.net/p/pixellight/code
\end{lstlisting}
Example:
\begin{lstlisting}[language=sh]
git clone ssh://bob42@git.code.sf.net/p/pixellight/code
\end{lstlisting}
...
\begin{lstlisting}[language=sh]
git://git.code.sf.net/p/pixellight/code
\end{lstlisting}
...
\end{quotation}
you might want to check your firewall settings. Additionally you can browse the Git repository within your web browser by opening \url{http://sourceforge.net/p/pixellight/code/} to ensure that the server is not down (doesn't happen that often).

You can use the following \ac{RSS} feed to get informed automatically about changes within the public Git repository:
\begin{lstlisting}[language=sh]
http://sourceforge.net/p/pixellight/code/feed/
\end{lstlisting}
