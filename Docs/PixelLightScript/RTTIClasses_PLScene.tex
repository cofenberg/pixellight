...
\paragraph{<RTTI object> Clone()}
Write \emph{<RTTI object>:Clone()} in order to create a clone of this scene node within the scene container this scene node is in. Returns the created clone of this scene node within the same scene container the original scene node is in, null pointer on error. Scene nodes and scene node modifiers with a set \emph{Automatic}-flag will not be cloned. The debug flags of the created clone are set to 0.
