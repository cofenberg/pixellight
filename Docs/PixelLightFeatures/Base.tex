...
\section{PLRenderer}
\begin{itemize}
\item{Dynamic \ac{API} design\footnote{The PixelLight \ac{SDK} comes with Null and OpenGL (using FreeType (\url{http://www.freetype.org/}) for font support) backends, within the repository there's also a OpenGL ES 2.0 backend and an experimental Direct3D 9 backend}}
...
\item{1D, 2D, rectangle\footnote{Predecessor of the modern and universal \ac{NPOT} textures \ac{GPU} feature}, 3D and cube textures}
...
\item{Abstract \ac{GPU} program interface with support for vertex, geometry and fragment shaders}
\item{The OpenGL renderer backend has a build in \ac{GPU} program interface implementation for \ac{GLSL}}
\item{The OpenGL renderer backend has an optional \ac{GPU} program interface implementation for Cg (\url{http://developer.nvidia.com/object/cg_toolkit.html}) via plugin}
\item{\ac{UBO} support}
...
\item{\ac{GPU} program generator class for Über-shaders}
\end{itemize}
