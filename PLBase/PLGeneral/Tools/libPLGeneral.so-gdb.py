...
class PLStringPrinter:
	...
	def to_string(self):
		...
		if bufType == 0:
			ty = gdb.lookup_type('PLGeneral::StringBufferASCII').pointer();
			return self.val['m_pStringBuffer'].cast(ty)['m_pszString']
		elif bufType == 1:
			ty = gdb.lookup_type('PLGeneral::StringBufferUnicode').pointer();
...
def build_dictionary ():
	pretty_printers_dict[re.compile('^PLGeneral::String$')] = lambda val: PLStringPrinter(val)
