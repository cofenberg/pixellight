...
class SNMPostProcessDepthOfField : public SNMPostProcess {
	...
	//[ RTTI interface                                        ]
	//[-------------------------------------------------------]
	pl_class(PLS_RTTI_EXPORT, SNMPostProcessDepthOfField, "PLScene", PLScene::SNMPostProcess, "Depth of field camera scene node post process modifier class")
	...
	public:
		...
		*    Owner scene node
		*/
		PLS_API SNMPostProcessDepthOfField(SceneNode &cSceneNode);

		/**
		*  @brief
		*    Destructor
		*/
		PLS_API virtual ~SNMPostProcessDepthOfField();
