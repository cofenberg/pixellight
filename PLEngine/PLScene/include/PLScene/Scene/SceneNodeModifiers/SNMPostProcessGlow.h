...
class SNMPostProcessGlow : public SNMPostProcess {
	...
	//[ RTTI interface                                        ]
	//[-------------------------------------------------------]
	pl_class(PLS_RTTI_EXPORT, SNMPostProcessGlow, "PLScene", PLScene::SNMPostProcess, "Glow camera scene node post process modifier class")
	...
	public:
		...
		*    Owner scene node
		*/
		PLS_API SNMPostProcessGlow(SceneNode &cSceneNode);

		/**
		*  @brief
		*    Destructor
		*/
		PLS_API virtual ~SNMPostProcessGlow();
