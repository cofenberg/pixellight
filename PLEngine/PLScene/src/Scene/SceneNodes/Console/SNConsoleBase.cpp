...
#include "PLScene/Scene/SceneNodes/Console/ConsoleDefaultCommands.h"
...
void SNConsoleBase::ProcessKeyMessage()
{
	...
	if ((pController && pController->GetActive()) || !pController) {
		...
		if (pKeyboard) {
			...
			if (pApplication) {
				bActivated = pApplication->GetConfig().GetVar("PLScene::EngineGeneralConfig", "ConsoleActivated").GetBool();
			}
