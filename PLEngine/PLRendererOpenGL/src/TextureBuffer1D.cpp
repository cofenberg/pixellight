				...
				if (!m_nNumOfMipmaps && bMipmaps) {
					...
					if (cRendererOpenGL.IsGL_SGIS_generate_mipmap()) {
						...
						if (bUsePreCompressedData)
							...
							glTexImage1D(GL_TEXTURE_1D, 0, *pAPIPixelFormat, m_nSize, 0, nAPIImageFormatUncompressed, nImageDataFormatUncompressed, pImageBuffer->GetData());
						...
						gluBuild1DMipmaps(GL_TEXTURE_1D, *pAPIPixelFormat, m_nSize, nAPIImageFormatUncompressed, nImageDataFormatUncompressed, pImageBuffer->GetData());
					...
					for (; nLevel<=m_nNumOfMipmaps; nLevel++) {
						...
						if (pMipmapImageBuffer) {
							...
							if (bUsePreCompressedData && pMipmapImageBuffer->HasCompressedData())
								...
								glTexImage1D(GL_TEXTURE_1D, nLevel, *pAPIPixelFormat, nSize, 0, nAPIImageFormatUncompressed, nImageDataFormatUncompressed, pMipmapImageBuffer->GetData());
