...
//[ RTTI interface                                        ]
//[-------------------------------------------------------]
pl_implement_class(SRPVolumetricFog)
	...
	if (!m_pProgramGenerator || m_pProgramGenerator->GetShaderLanguage() != sShaderLanguage) {
		...
		if (sShaderLanguage == "GLSL") {
			...
			if (cRenderer.GetAPI() == "OpenGL ES 2.0") {
				...
				m_pProgramGenerator = new ProgramGenerator(cRenderer, sShaderLanguage, Shader::RemovePrecisionQualifiersFromGLSL(sVolumetricFog_GLSL_VS), "110", Shader::RemovePrecisionQualifiersFromGLSL(sVolumetricFog_GLSL_FS), "110");
			}
