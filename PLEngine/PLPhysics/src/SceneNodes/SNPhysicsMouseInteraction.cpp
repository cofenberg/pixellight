			...
			if (pMouse) {
				...
				if (pContainer) {
					...
					if (pPhysicsWorld) {
						...
						// On picking end
						if ( (!pMouse->Middle.IsPressed() && m_bPicking) || (!m_bPicking && m_pPickedPhysicsBody)) {
							// Reset picking data
						...
						// On picking start
						} else if (pMouse->Middle.IsPressed() && !m_bPicking) {
						...
						// On picking
						} else if (pMouse->Middle.IsPressed() && m_bPicking && m_pPickedPhysicsBody) {
							// [TODO] Find a solution that works without using PLCore::ConsoleApplication::GetApplication()
							// [TODO] PLGui::GuiApplication type check
							// Get the main window of the application
							if (PLCore::ConsoleApplication::GetApplication() && ((PLGui::GuiApplication*)PLCore::ConsoleApplication::GetApplication())->GetMainWindow()) {
								...
								if (((PLGui::GuiApplication*)PLCore::ConsoleApplication::GetApplication())->GetMainWindow()->GetContentWidget()->GetMousePos(vMousePos)) {
									...
									if (pKeyboard) {
										...
										if (pKeyboard->KeyAlt.IsPressed()) {
											...
											// Add 'kick' force :)
											Vector3 vDir = (vV1 - pCamera->GetTransform().GetPosition()).Normalize();
											if (pKeyboard->KeyShift.IsPressed())
												vPickedForce += vDir*ThrowForce*2;
											else
												vPickedForce += vDir*ThrowForce;
										}
									}

									// The mouse wheel can be used to push/pull the target
									const float fWheelDelta = pMouse->Wheel.GetValue();
									if (fWheelDelta) {
